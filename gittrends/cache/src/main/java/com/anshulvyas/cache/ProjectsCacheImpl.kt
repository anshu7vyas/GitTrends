package com.anshulvyas.cache

import com.anshulvyas.cache.db.ProjectsDatabase
import com.anshulvyas.cache.mapper.CachedProjectMapper
...
class ProjectsCacheImpl @Inject constructor(
    ...
    // add call to DAO to delete all projects
    ...
    override fun setLastCacheTime(lastCache: Long): Completable {
        return Completable.defer {
            projectsDatabase.configDao().insertConfig(Config(lastTimeCache = lastCache))
            Completable.complete()
        }
    }
    ...
    override fun isProjectsCacheExpired(): Single<Boolean> {
        ...
        val expirationTime = (60 * 10 * 1000).toLong() // 1 day
        return projectsDatabase.configDao().getConfig()
                .single(Config(lastTimeCache = 0))
                .map {
